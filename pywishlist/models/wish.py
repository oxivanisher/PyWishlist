...
class Wish(Base):
    ...
    source = relationship('WishUser', foreign_keys=[sourceId], backref=backref('source', lazy='dynamic'))
    creationDate = Column(Integer, unique=False)
    destinationId = Column(Integer, ForeignKey('user.id'))
    destination = relationship('WishUser', foreign_keys=[destinationId], backref=backref('destination', lazy='dynamic'))
    hiddenId = Column(Integer, ForeignKey('user.id'))
    hiddenBy = relationship('WishUser', foreign_keys=[hiddenId], backref=backref('hiddenby', lazy='dynamic'))
