...
def admin_user_management_updateemail(userId):
    ...
    if myUser:
        ...
        log.info("[System] Email address of '%s' was changed to: %s" % (myUser.name, myUser.email))
        db_session.merge(myUser)
    try:
        runQuery(db_session.commit)
    except Exception as e:
        log.warning("[System] SQL Alchemy Error on update email address admin: %s" % (e))
...
def admin_user_management_updatename(userId):
    ...
    if myUser:
        ...
        log.info("[System] Name of '%s' was changed to: %s" % (myUser.email, myUser.name))
        db_session.merge(myUser)
    try:
        runQuery(db_session.commit)
    except Exception as e:
        log.warning("[System] SQL Alchemy Error on update name admin: %s" % (e))
    return redirect(url_for('admin_user_management'))
